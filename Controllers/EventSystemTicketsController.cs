        ...
        public IActionResult CreateWithAutomation(Ticket ticket)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (venue != null)
                {
                    ...
                    // Check if all seats already exist for the venue
                    bool allSeatsExist = _context.Seats
                        .Where(s => s.VenueId == ticket.VenueId && s.SeatNumber <= seatCapacity)
                        .Count() == seatCapacity;

                    if (!allSeatsExist)
                    {
                        // Create new seats for the venue
